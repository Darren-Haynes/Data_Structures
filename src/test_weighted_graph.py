...
def test_add_node_creates_value_error(empty_g):
    ...
    with pytest.raises(ValueError):
        empty_g.add_edge('A', 'A', 5)

def test_add_node_errors_with_non_number_weight(empty_g):
    """If weight not float or int valueerror should be raised."""
    with pytest.raises(ValueError):
        empty_g.add_edge('A', 'B', '5')


def test_add_edge_int_weight_should_exist(empty_g):
    """If weigh is int, should be added to dict."""
    empty_g.add_edge('A', 'B', 5)
    assert empty_g.graph['A']['B'] == 5


def test_add_edge_float_weight_should_exist(empty_g):
    """If weigh is float, should be added to dict."""
    empty_g.add_edge('A', 'B', 5.5)
    assert empty_g.graph['A']['B'] == 5.5
